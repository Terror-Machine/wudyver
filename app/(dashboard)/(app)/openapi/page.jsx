          ...
          <SimpleBar style={{ maxHeight: 'calc(80vh - 120px)' }} className="p-0">
            <div className="p-3 sm:p-4 space-y-3 sm:space-y-4">
                  ...
                  {apiQueryParams.map((param, index) => (
                    <div key={param.id} className="flex items-center gap-2">
                      ...
                      <Button
                        ...
                        className={`${buttonSecondaryClass} flex-none text-red-500 hover:text-red-600 dark:text-red-400 dark:hover:text-red-300 p-1.5`}
                        disabled={apiQueryParams.length === 1}
                        title="Remove parameter"
                      />
                    </div>
                  ))}
                  {apiQueryParams.length === 0 && (
                      <p className="text-xs text-slate-500 dark:text-slate-400 text-center py-2">No query parameters.</p>
                  )}
                </div>
              )}
              ...
              {(apiRequestMethod === "POST" || apiRequestMethod === "PUT" || apiRequestMethod === "PATCH") && (
                <div className="space-y-2 border border-slate-200 dark:border-slate-700/60 p-3 rounded-md bg-slate-50 dark:bg-slate-800/50">
                  <label htmlFor="apiRequestBody" className="block text-xs font-medium text-teal-700 dark:text-teal-300 mb-2 flex items-center">
                    <Icon icon="ph:code-duotone" className="mr-1 text-base"/> Request Body (JSON)
                  </label>
                  <textarea
                    id="apiRequestBody"
                    value={apiRequestBody}
                    onChange={(e) => setApiRequestBody(e.target.value)}
                    className={`${inputBaseClass} font-mono h-32 resize-y`}
                    placeholder="Enter JSON request body here..."
                  ></textarea>
                  <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Ensure valid JSON format.</p>
                </div>
              )}
              ...
              <div className="space-y-2 border border-slate-200 dark:border-slate-700/60 p-3 rounded-md bg-slate-50 dark:bg-slate-800/50">
                <div className="flex justify-between items-center mb-2">
                  <label className="block text-xs font-medium text-teal-700 dark:text-teal-300 flex items-center">
                    <Icon icon="ph:terminal-duotone" className="mr-1 text-base"/> cURL Command
                  </label>
                  <Button
                    onClick={handleCopyCurl}
                    text={copiedCurl ? "Copied!" : "Copy"}
                    icon={copiedCurl ? "ph:check-circle-duotone" : "ph:copy-duotone"}
                    className={`${buttonSecondaryClass} text-[11px] px-2 py-1`}
                  />
                </div>
                <SyntaxHighlighter
                  language="bash"
              ...
              <div className="space-y-2 border border-slate-200 dark:border-slate-700/60 p-3 rounded-md bg-slate-50 dark:bg-slate-800/50">
                <div className="flex justify-between items-center mb-2">
                  <label className="block text-xs font-medium text-teal-700 dark:text-teal-300 flex items-center">
                    <Icon icon="ph:arrow-square-down-duotone" className="mr-1 text-base"/> API Response
                  </label>
                  <div className="flex gap-2 items-center">
                    {apiResponseStatus && (
                      <span className={`text-[10px] sm:text-xs font-bold px-2 py-1 rounded-full ${
                        apiResponseStatus >= 200 && apiResponseStatus < 300 ? 'bg-green-100 text-green-800 dark:bg-green-900/50 dark:text-green-300' :
                        apiResponseStatus >= 400 && apiResponseStatus < 500 ? 'bg-red-100 text-red-800 dark:bg-red-900/50 dark:text-red-300' :
                        apiResponseStatus >= 500 ? 'bg-purple-100 text-purple-800 dark:bg-purple-900/50 dark:text-purple-300' :
                    ...
                    {apiResponseContentType && (
                      <span className="text-[10px] sm:text-xs font-medium px-2 py-1 rounded-full bg-slate-100 text-slate-600 dark:bg-slate-700/50 dark:text-slate-300">
                        Type: {apiResponseContentType.split(';')[0]}
                      </span>
                    )}
                    <Button
                      ...
                      className={`${buttonSecondaryClass} text-[11px] px-2 py-1`}
                      disabled={!apiResponse && apiResponseStatus !== 204}
                    />
                  </div>
                </div>
                <div className="relative border border-slate-300 dark:border-slate-600 rounded-md overflow-hidden min-h-[100px]">
                  {renderApiResponse()}
                </div>
                {apiResponseHeaders && (
                    <div className="mt-2">
                        <p className="text-xs font-medium text-slate-600 dark:text-slate-300 flex items-center mb-1">
                            <Icon icon="ph:info-duotone" className="mr-1 text-base"/> Response Headers:
                        </p>
                        <SyntaxHighlighter
                            language="json"
                            style={syntaxHighlighterTheme}
                            customStyle={{ margin: 0, padding: '0.5rem 1rem', borderRadius: '0px', maxHeight: '100px', fontSize: '10px' }}
                            showLineNumbers={false}
                            wrapLines={true}
                            className="simple-scrollbar"
                        >
