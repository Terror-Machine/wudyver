...
const PostsPage = () => {
  ...
  const handleAddPost = async () => {
    if (!newPost.title || !newPost.content || !newPost.author) {
      toast.warn("Mohon isi Judul, Konten, dan Penulis");
      return;
    }
    setLoading(true);
    try {
      ...
      if (result.success) {
        ...
        setNewPost({ title: "", content: "", author: "", authorAvatar: "" });
        setShowAddPostModal(false);
        fetchPosts();
      } else {
        toast.error(result.message || "Gagal menambahkan postingan");
      }
    } catch (error) {
        ...
        >
          <div className="p-4 sm:p-6 border-b border-slate-200 dark:border-slate-700/60">
            ...
            <p className="text-sm text-center sm:text-left text-slate-500 dark:text-slate-400 mt-2 ml-0 sm:ml-[calc(2.5rem+1rem)] md:ml-[calc(3rem+1rem)]">
              Kelola dan bagikan artikel dan postingan Anda!
            </p>
          </div>
              ...
              {!loading && posts.length > 0 && (
                <div className="space-y-3 sm:space-y-4">
                  {posts.map((post) => {
                    ...
                    return (
                      <div key={post._id} className="bg-white dark:bg-slate-800/60 p-3 sm:p-4 rounded-lg border border-slate-200 dark:border-slate-700/50 shadow-sm hover:shadow-md transition-shadow duration-150">
                        ...
                        <div className="bg-slate-100 dark:bg-slate-900/70 rounded-md p-2.5 sm:p-3 border border-slate-200 dark:border-slate-700/80 relative mt-1.5">
                          ...
                          <div className="flex flex-col sm:flex-row items-center justify-between mt-2.5 gap-1.5 sm:gap-2">
                            ...
                            {contentSegments.length > 1 && (
                              hasMoreSegments ? (
                                <Button
                                  onClick={() => showNextContentSegment(post._id)}
                                ...
                                <Button
                                  onClick={() => resetContentView(post._id)}
        ...
        footerContent={
          <div className="flex justify-end space-x-2">
            ...
            <Button
              ...
              text="Simpan Postingan"
              className={`${buttonGradientBase} bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600`}
              disabled={loading || !newPost.title || !newPost.content || !newPost.author}
      ...
      >
        <div className="space-y-3 text-slate-800 dark:text-slate-100 p-0.5">
          <Textinput
            ...
            description="Judul yang menarik untuk postingan Anda."
          />
          <Textinput
            label="Nama Penulis *"
            ...
            className={inputBaseClass}
            labelClass={labelBaseClass}
            description="Nama penulis postingan."
          />
          <Textinput
            label="URL Avatar Penulis (Opsional)"
            placeholder="https://example.com/avatar.png"
            value={newPost.authorAvatar}
            onChange={(e) => setNewPost({ ...newPost, authorAvatar: e.target.value })}
            className={inputBaseClass}
            labelClass={labelBaseClass}
            description="URL gambar untuk avatar penulis."
          />
          <div>
            <label htmlFor="newPostContent" className={labelBaseClass}>
              Konten Postingan *
            </label>
            <textarea
              id="newPostContent"
            ...
            />
            <p className="text-[10px] text-slate-500 dark:text-slate-400 mt-0.5">Isi dengan konten artikel atau postingan yang ingin Anda bagikan.</p>
          </div>
        </div>
      </Modal>
