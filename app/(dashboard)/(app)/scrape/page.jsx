...
const GitHubFileScraperPage = ({ initialPath = "pages/api" }) => {
  ...
  return (
    <div className="w-full px-2 sm:px-4 py-6"> {/* Matched CekResiPage */}
      ...
      <Card
        bodyClass="relative p-0 h-full overflow-hidden flex flex-col" // Kept flex-col for internal layout
        className="w-full border border-teal-500/50 dark:border-teal-600/70 rounded-xl shadow-lg bg-white text-slate-800 dark:bg-slate-800/50 dark:text-slate-100 backdrop-blur-sm bg-opacity-80 dark:bg-opacity-80" // Matched CekResiPage
      >
        {/* Header Card - Adapted from CekResiPage */}
        ...
        {/* Loading and Error States - Unchanged structurally, will fit new card style */}
        ...
        {!loadingList && !error && (
          // md:h-[calc(100vh-xxx)] might need fine-tuning based on final header height.
          // For now, let the content define its height within the flex-col Card.
          <div className="flex-grow md:flex md:min-h-0"> {/* Ensure flex-grow and min-h-0 for flex children */}
            <div className="w-full md:w-2/5 lg:w-1/3 border-b md:border-b-0 md:border-r border-slate-200 dark:border-slate-700/60 flex flex-col">
              {/* Search input styled like CekResiPage form blocks */}
              ...
              {totalPages > 1 && (
                <div className="p-2.5 sm:p-3 border-t border-slate-200 dark:border-slate-700/60 flex flex-col items-center gap-2 sm:flex-row sm:justify-between text-xs bg-slate-100/70 dark:bg-slate-800/40"> {/* Styled like CekResi input wrappers */}
            ...
            <div className="hidden md:flex md:w-3/5 lg:w-2/3 bg-slate-50 dark:bg-slate-800/30 items-center justify-center p-6 rounded-br-xl md:rounded-bl-none flex-grow"> {/* Added flex-grow to fill space */}
      ...
      {/* Modal - Unchanged, styling is self-contained */}
