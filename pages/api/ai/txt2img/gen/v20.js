...
class AiGreemAutomator {
  ...
  async uploadImage(base64ImageDataUri) {
    try {
      console.log("[🌀 UPLOAD] Mengunggah gambar ke Catbox...");
      if (!base64ImageDataUri || !base64ImageDataUri.startsWith("data:")) {
        throw new Error("Input must be a valid data URI (e.g., data:image/png;base64,...).");
      }
      const requestData = {
        file: base64ImageDataUri
      };
      const {
        data: uploadResponse
      ...
      if (!uploadResponse.result) {
        throw new Error("Upload failed: No result in response.");
      }
      console.log(`[✅ BERHASIL] Gambar berhasil diunggah: ${uploadResponse.result}`);
